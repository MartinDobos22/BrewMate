  ...
  messages: [
    {
      role: 'system',
      content:
        'You are a helpful assistant that corrects OCR text while preserving meaning.',
    },
    {
      role: 'user',
...
const buildFilterRecipePayload = (
  ...
  const userContextBlock = userQuestionnaire
    ? `\n\nProfil používateľa (z dotazníka):\n` +
      `- Chuťový vektor: ${JSON.stringify(
        userQuestionnaire.tasteVector || {},
      )}\n` +
      `- Tolerancie: ${JSON.stringify(
        userQuestionnaire.toleranceVector || {},
      )}\n` +
      `- Otvorenosť: ${userQuestionnaire.openness || 'neznáme'}\n` +
      `- Zhrnutie preferencií: ${
        userQuestionnaire.profileSummary || 'nedostupné'
      }\n` +
    ...
    messages: [
      ...
      },
      {
        ...
        content: `Chuťový profil:\n${JSON.stringify(
          ...
          null,
          2,
        )}\n\nVybraný spôsob prípravy: ${selectedPreparation}\nVlastná príprava od používateľa: ${
          customPreparationText || 'nie'
...
const buildEspressoRecipePayload = (
  ...
  const userContextBlock = userQuestionnaire
    ? `\n\nProfil používateľa (z dotazníka):\n` +
      ...
      `- Preferencia mlieka: ${
        userQuestionnaire.milkPreference || 'neznáme'
      }\n` +
      `Prispôsob recept chuťovým preferenciám používateľa. Ak má nízku toleranciu na niektorú os ` +
...
const matchScoreToTier = score => {
  if (score >= MATCH_TIER_THRESHOLDS.perfect) {
    return 'perfect_match';
  }
  if (score >= MATCH_TIER_THRESHOLDS.great) {
    return 'great_choice';
  }
  if (score >= MATCH_TIER_THRESHOLDS.worthTrying) {
    return 'worth_trying';
  }
  if (score >= MATCH_TIER_THRESHOLDS.experiment) {
    return 'interesting_experiment';
  }
  return 'not_for_you';
};
...
const buildVectorMatchReason = (coffeeVector, userVector, tolerance) => {
  ...
  for (const axis of TASTE_AXES) {
    const diff = Math.abs(
      clampAxis(userVector[axis]) - clampAxis(coffeeVector[axis]),
    );
    ...
    if (diff <= 15) {
      ...
      diff >= 40 &&
      (tolerance[axis] === 'dislike' || tolerance[axis] === 'neutral')
    ) {
...
const computeCalibrationOffset = feedbackRows => {
  ...
  const clamped = Math.max(
    ...
    Math.min(CALIBRATION_MAX_OFFSET, rawOffset),
  );
...
const computeMatchPrediction = ({
  ...
  if (hasQuestionnaire && coffeeVector) {
    ...
    for (const axis of TASTE_AXES) {
      ...
      const weight =
        TOLERANCE_WEIGHTS[toleranceLevel] || TOLERANCE_WEIGHTS.neutral;
      ...
      if (diff <= 15) {
        ...
        diff >= 40 &&
        (toleranceLevel === 'dislike' || toleranceLevel === 'neutral')
      ) {
        status = 'conflict';
      }
      axes.push({
    ...
    const score = Math.max(
      0,
      Math.min(99, preCalibrationScore - calibrationOffset),
    );
...
const computeCoffeeProfileMatch = ({
  coffeeProfile,
  userQuestionnaire,
  calibration,
}) => {
  ...
  const tolerance = userQuestionnaire.toleranceVector || {};
  const opennessRaw = String(
    userQuestionnaire.openness || 'moderate',
  ).toLowerCase();
  const openness = ['conservative', 'moderate', 'adventurous'].includes(
    opennessRaw,
  )
    ? opennessRaw
    : 'moderate';
...
const buildCoffeeMatchTextPayload = (
  questionnaire,
  coffeeProfile,
  vectorResult,
) => ({
  ...
  messages: [
    ...
    },
    {
      role: 'user',
      content:
        `Deterministické skóre: ${vectorResult.matchScore}/100 (tier: ${vectorResult.matchTier}).\n` +
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (cached) {
        log.info('PhotoAnalysis cache hit', {
      ...
      log.error('PhotoAnalysis unexpected error', {
  ...
  try {
    ...
    if (brewPath !== 'espresso' && brewPath !== 'filter') {
      return sendError(
        res,
        'validation_error',
        'brewPath must be "espresso" or "filter".',
      );
    }
    ...
    try {
      ...
      console.warn(
        '[PhotoRecipe] Failed to load personalization data',
        dbError?.message,
      );
    }
    ...
    if (brewPath === 'espresso') {
      ...
      if (!drinkType || typeof drinkType !== 'string') {
        return sendError(
          res,
          'validation_error',
          'drinkType is required for espresso path.',
        );
      }
      if (!machineType || typeof machineType !== 'string') {
        return sendError(
          res,
          'validation_error',
          'machineType is required for espresso path.',
        );
      }
      ...
      if (
        ...
        return sendError(
          res,
          'validation_error',
          'At least one of targetDoseG or targetYieldG is required.',
        );
      }
      ...
      effectivePreparation =
        typeof selectedPreparation === 'string' && selectedPreparation.trim()
          ? selectedPreparation.trim()
          : typeof customPreparationText === 'string' &&
      ...
      if (
        ...
        return sendError(
          res,
          'validation_error',
          'At least one of targetDoseG or targetWaterMl is required.',
        );
      }
    ...
    log.error('PhotoRecipe unexpected error', {
      error: error?.message || error,
    });
    return next(error);
  }
...
router.post('/api/coffee-profile', aiRateLimit, async (req, res, next) => {
  try {
    ...
    log.error('CoffeeProfile unexpected error', {
      error: error?.message || error,
    });
    return next(error);
  }
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      log.error('CoffeeQuestionnaire unexpected error', {
        error: error?.message || error,
      });
      return next(error);
    }
  ...
  try {
    ...
    if (!questionnaire || !coffeeProfile) {
      return sendError(
        res,
        'validation_error',
        'questionnaire and coffeeProfile are required.',
      );
    }
    ...
    if (memoryHit) {
      log.info('CoffeeMatch shared cache hit', {
        cacheKey: cacheKey.slice(0, 12),
      });
    ...
    try {
      ...
      if (dbHit.rows.length > 0) {
        ...
        const cachedMatch =
          typeof row.match === 'string' ? JSON.parse(row.match) : row.match;
        log.info('CoffeeMatch DB cache hit', {
          cacheKey: cacheKey.slice(0, 12),
        });
      ...
      log.warn('CoffeeMatch DB cache lookup failed', {
    ...
    try {
      ...
      console.warn(
        '[CoffeeMatch] Failed to load calibration feedback',
        dbError?.message,
      );
    }
    ...
    if (vectorResult.hasVector) {
      const { content: textContent } = await callOpenAI({
        apiKey: openAiApiKey,
        payload: buildCoffeeMatchTextPayload(
          questionnaire,
          coffeeProfile,
          vectorResult,
        ),
      ...
      match = {
        ...
        baristaSummary:
          typeof texts?.baristaSummary === 'string' ? texts.baristaSummary : '',
        laymanSummary:
          typeof texts?.laymanSummary === 'string' ? texts.laymanSummary : '',
        keyMatches: vectorResult.keyMatches,
        keyConflicts: vectorResult.keyConflicts,
        suggestedAdjustments:
          typeof texts?.suggestedAdjustments === 'string'
            ? texts.suggestedAdjustments
            : '',
        adventureNote:
          typeof texts?.adventureNote === 'string' ? texts.adventureNote : '',
    ...
    try {
      ...
      console.warn(
        '[CoffeeMatch] Failed to persist match cache',
        dbError?.message,
      );
    }
    ...
    log.error('CoffeeMatch unexpected error', {
      error: error?.message || error,
    });
    return next(error);
  }
